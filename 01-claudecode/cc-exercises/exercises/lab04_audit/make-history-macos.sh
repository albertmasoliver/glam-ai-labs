...
# `sed -i` differs between GNU and BSD (macOS), so rewrite via a temp file.
sed 's/^    assert status in (200, 400)$/    assert status == 400/' \
  tests/test_web.py > "$keep/patched" && mv "$keep/patched" tests/test_web.py
