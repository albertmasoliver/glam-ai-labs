...
add(n=11, slug="build_trainer", title="Wire up the Trainer",
    ...
    main=['# Assemble the Trainer and actually fine-tune for 2 steps on a tiny dataset (CPU-ok).',
...
add(n=12, slug="predict", title="Use the fine-tuned model (from logits)",
    ...
    sig="def solve(text):",
    blank=['    name = "distilbert-base-uncased-finetuned-sst-2-english"',
    ...
    sol=['    name = "distilbert-base-uncased-finetuned-sst-2-english"',
         '    tok = AutoTokenizer.from_pretrained(name)',
         '    model = AutoModelForSequenceClassification.from_pretrained(name)',
         '    inp = tok(text, return_tensors="pt", padding=True, truncation=True)',
         '    with torch.no_grad():',
